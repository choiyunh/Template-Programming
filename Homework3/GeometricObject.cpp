...
// Mouse cursor is in circle ?
bool GeometricObject::InsideCircle(double &x, double &y, const int i, const int j, const int r)
{
	y = height - y;
...
// For mouse handling ( change circle's color )
void GeometricObject::ChangeCircle(const int& i, const int& j, const int& r, GLFWwindow* window)
{
	double xpos, ypos;
	...
	circle->drawCircle(i, j, r, 1.0f, 0.0f, 0.0f);

	if (InsideCircle(xpos, ypos, i, j, r) == true)
	{
		circle->drawCircle(i, j, r, 0.0f, 0.0f, 1.0f);
	}
}
